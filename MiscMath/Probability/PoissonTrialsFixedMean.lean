...
statements against the informal claim above on a best-effort basis. They are the five
definition-free statements of `## The statements` below; the natural-form theorems those
are discharged by, `hoeffding_thm3`, `hoeffding_thm3_eq_iff`, `hoeffding_cor21`,
`hoeffding_cor21_min`, `hoeffding_thm4` and `hoeffding_thm4_range`; the three lemmas of
`PoissonTrialsFixedMean.Bridge` that tie this library's definitions to Mathlib's,
`bernExp_const_eq_integral_binomial`, `binTail_eq_binomial_real_Iic` and
`bernExp_eq_integral_pi_bernoulliMeasure`; and the seven declarations that pin down what
is and is not claimed, `hoeffding_thm3_eq_needs_strict`, `hoeffding_thm3_needs_le_one`,
`hoeffding_thm3_needs_nonneg`, `thm4_lower_needs_le_sub_one`, `thm4_gap_subsingleton`,
`thm4_gap_eq_floor` and `thm4_gap_empty_of_natCast` — each read together with the
definitions it is stated through, `bernWt`, `bernExp`, `secondDiff`, `tailLe`, `binTail`,
`InBox` and `AtMostOneInteriorValue`. The statements and their proofs were generated by
Claude, and are kernel-verified and axiom-audited; the proofs are read by nobody. Every
other lemma and definition in this file and in the modules under
`MiscMath/Probability/PoissonTrialsFixedMean/` is proof, and may be read by no one. A
best-effort read is not a review — satisfy yourself that the statement says what you need
before relying on it. See the repository README.

Before that read, the five statements of `## The statements`, and those only, were read
back blind: rendered into English by an agent given them and nothing else — no informal
statement, no source, no docstring. The rendering agreed with the informal
