...
claims. That is the one thing no automated check can do, and it is why the statements a
result advertises get a best-effort read from the author and why every result file
