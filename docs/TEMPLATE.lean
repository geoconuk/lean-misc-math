...
statements — <name each one, wherever it lives, and any definition it is stated through> —
against the informal claim above on a best-effort basis. The statements and their proofs
were generated by Claude, and are kernel-verified and axiom-audited; the proofs are read by
nobody. Every other lemma and definition here is proof, and may be read by no one. A
